...
namespace Toko_Obat
{
    public partial class Apoteker : Form
    {
        ...
        private void search_TextChanged(object sender, EventArgs e)
        {
            ...
            {
                conn.Open();
                SqlCommand cmd = new SqlCommand("SELECT * FROM Apoteker Where Nama_Apoteker LIKE  '%" + search.Text + "%' OR Id_Apoteker LIKE '%" + search.Text + "%' " +
                    "OR Alamat LIKE '%" + search.Text + "%' ", conn);
